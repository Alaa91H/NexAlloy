...
### SonyLIV — Disable AppsFlyer tracking

The public `DisableAppsFlyerPatch.kt` and `Fingerprints.kt` in `durgesh0505/chiggi_morphe_patches` were reviewed as text only. The source identifies one exact private-final, zero-argument Boolean gate: `Lcom/sonyliv/Analytics/AppsFlyerManager;->isAppsFlyerSupported(): boolean`. The source states that SonyLIV's AppsFlyer initialization and event calls are guarded by that method; returning `false` prevents that reviewed attribution SDK path from being initialized.

The local adapter applies `false` only when the exact class descriptor, method name, private-final access flags, Boolean return type, and zero-argument signature resolve in `com.sonyliv`. A changed or absent target installs no hook. The layer does not alter accounts, authentication, subscriptions, purchases, licensing, content eligibility, DRM, downloads, integrity, geolocation, or other telemetry SDKs. No external archive, extension, or payload is downloaded, interpreted, or executed.

### SonyLIV — Hide promo banners

The public `HidePromoBannersPatch.kt` and `Fingerprints.kt` in `durgesh0505/chiggi_morphe_patches` were reviewed as text only. The source identifies two public instance methods on `Lcom/sonyliv/data/clevertap/CtNativeDisplayRepositoryImpl;`, both with one `String` parameter: `getCTListRowData(String): CTListRowModel` and `getCTRowData(String): CTBannerCardRowModel`. Returning `null` from these exact methods suppresses only the reviewed CleverTap home promo tray and banner-card models; the upstream source explicitly states that content rows and overlays/pop-ups remain outside this layer.

The local adapter uses exact class descriptors, method names, object return types, and the one-String parameter contract. A changed or absent target installs no hook. It hides promotional UI only and does not alter accounts, authentication, subscriptions, purchases, licensing, entitlement, content eligibility, DRM, downloads, integrity, tracking, or server-side access controls. No external archive, extension, or payload is downloaded, interpreted, or executed.

### Arrow Puzzle — Disable analytics

The public `DisableAnalyticsPatch.kt` and `Fingerprints.kt` in `durgesh0505/chiggi_morphe_patches` were reviewed as text only. The source identifies exactly two void event-transmit chokepoints for `com.easybrain.arrow.puzzle.game`: public-static-final `Lcom/easybrain/analytics/unity/AnalyticsPlugin;->AnalyticsSendEvent(String, String): void` and public-final `Lxb/i;->j(Lcom/easybrain/analytics/event/b;): void`. The upstream description confines the change to event transmit methods and explicitly preserves analytics-controller initialization and adapter registration.

The local adapter skips only these exact class, method, access-flag, parameter, and void-return contracts. A changed or absent target installs no hook. It does not alter gameplay, rewards, advertisements, accounts, authentication, purchases, subscriptions, licensing, content eligibility, DRM, device integrity, or server-side controls. No external archive, extension, or payload is downloaded, interpreted, or executed.

### ZEE5 Android TV — Remove display ads

The public `RemoveDisplayAdsPatch.kt` and `Fingerprints.kt` in `durgesh0505/chiggi_morphe_patches` were reviewed as text only. The source identifies two client UI ad surfaces inside the Android TV package `com.graymatrix.did`: `HomeActivityViewModel.getNeedToShowNativeMastheadAd(): boolean`, which gates the native home masthead banner, and `FetchPlaybackUrlUseCase.toPauseAds(): PauseDisplayAds`, which creates the on-pause display overlay. The local adapter returns `false` and `null` only from those exact contracts.

This layer is limited to client-rendered display advertising. Video-ad removal from the same external source was deliberately deferred because it explicitly forces a path described as the ad-free/premium behavior. The local adapter does not alter accounts, authentication, subscriptions, purchases, licensing, entitlement, content eligibility, DRM, video playback, downloads, integrity, tracking, or server-side access controls. No external archive, extension, or payload is downloaded, interpreted, or executed.
